...
def seznam_stran(bod1, bod2, bod3):
    """Tato funkce vypočítá strany trojúhelníka
        ...
        """
    global list_strana
    list_strana = [((bod2[0] - bod1[0]) ** 2 + (bod2[1] - bod1[1]) ** 2) ** 0.5,
                   ((bod3[0] - bod2[0]) ** 2 + (bod3[1] - bod2[1]) ** 2) ** 0.5,
                   ((bod1[0] - bod3[0]) ** 2 + (bod1[1] - bod3[1]) ** 2) ** 0.5]
...
def check(sss):
    """Funkce určuje, zda trojúhelník se zvolenými vrcholí existuje (Součet dvou délek MUSÍ být větší než třetí strana).
        ...
        --------------
        `SystemExit()` :
            Funkce vrátí `SystemExit()` pokud neexistuje trojúhelníka s uvedenými vrcholy;
        `True` :
...
def check_prav(sss):
    """Funkce určuje, zda trojúhelník je pravoúhlý a oznamuje o tom uživatele
        ...
        --------------
        `True` :
            Funkce vrátí `True` pokud trojúhelník s uvedenými vrcholy je pravoúhlý, typ `boolean`;
        `False` :
            Funkce vrátí `False` pokud trojúhelník s uvedenými vrcholy není pravoúhlý, typ `boolean`
